...
import { clearCurrentGame, saveSoloGame } from "@/games/sans-le-dire/lib/game/persistence";

const DEFAULT_PLAYER_COUNT = 3;
const ACCENT = "var(--sld-accent)";
...
export function SoloSetupForm() {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const game = createSoloGame({ playerNames: fields.map((field) => field.value.trim()) }, cards);
      clearCurrentGame();
      saveSoloGame(game);
  ...
  return (
    <form className="player-form" onSubmit={submit} noValidate>
      ...
      <p className="setup-note">
        {MIN_SOLO_PLAYERS} à {MAX_SOLO_PLAYERS} joueurs · chacun devient maître une fois, 8 mots à
        la suite
      </p>
      {error ? (
