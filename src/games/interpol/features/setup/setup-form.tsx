...
import { createGame, MAX_PLAYERS, MIN_PLAYERS } from "@/games/interpol/lib/game/engine";
import { saveCurrentGame } from "@/games/interpol/lib/game/persistence";

const DEFAULT_PLAYER_COUNT = 4;
const ACCENT = "var(--interpol-accent)";
...
export function SetupForm() {
  ...
  return (
    <form className="player-form one-shot-player-form" onSubmit={submit} noValidate>
      <div className="player-list one-shot-player-list">
      ...
      <div>
        <p className="setup-note">
          {fields.length} joueurs · {fields[0]?.value || "le premier joueur"} sera le premier maître
          du jeu
        </p>
        {error ? (
