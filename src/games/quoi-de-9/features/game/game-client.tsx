...
import { Button, ButtonLink, ProgressDots } from "@/games/quoi-de-9/components/ui";
import { getQuestion, getTheme, questions, themes } from "@/games/quoi-de-9/data/questions";
import { DIFFICULTY_LEVELS, GAME_CONFIG, type DifficultyLevel } from "@/games/quoi-de-9/lib/game/config";
import {
...
import { clearCurrentGame, loadCurrentGame, saveCurrentGame } from "@/games/quoi-de-9/lib/game/persistence";
import { calculateTurnScore, formatScore, formatSignedScore } from "@/games/quoi-de-9/lib/game/scoring";
...
function GameHeader({ game, onAbandon }: { game: GameState; onAbandon: () => void }) {
...
function JokerConfirmationScreen({
  game,
  onCancel,
  onConfirm,
}: {
...
function ChoiceConfirmationScreen({
  theme,
  difficultyLevel,
  onCancel,
  onConfirm,
...
function PreparationScreen({ game, onContinue }: { game: GameState; onContinue: () => void }) {
  const master = game.teams[game.currentTeamIndex === 0 ? 1 : 0];
  return (
    <section className="flex flex-1 flex-col justify-center text-center">
      <div
...
function ScoreboardScreen({ game, onContinue }: { game: GameState; onContinue: () => void }) {
  const leader =
