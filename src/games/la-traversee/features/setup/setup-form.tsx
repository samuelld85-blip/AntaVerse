...
import { createGame, MAX_PLAYERS, MIN_PLAYERS } from "@/games/la-traversee/lib/game/engine";
import { saveCurrentGame } from "@/games/la-traversee/lib/game/persistence";

const DEFAULT_PLAYER_COUNT = 3;
const ACCENT = "var(--traversee-accent)";
...
export function SetupForm() {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.push("/la-traversee/partie");
    } catch {
    ...
    <form className="player-form" onSubmit={submit} noValidate>
      <div className="player-list">
        {fields.map((field, index) => (
          <ParticipantCard key={field.id} badge={index + 1} color={ACCENT} label={`Joueur ${index + 1}`} onRemove={isRemovable(index) ? () => removePlayer(field.id) : undefined} removeLabel={`Retirer le joueur ${index + 1}`} inputProps={{ value: field.value, onChange: (event) => updateName(field.id, event.target.value), placeholder: "Son prénom", "aria-label": `Nom du joueur ${index + 1}` }} />
        ))}
        <AddParticipantButton onClick={addPlayer} disabled={!canAddMore} color={ACCENT}>Ajouter un joueur</AddParticipantButton>
      </div>
      <div>
        <p className="setup-note">{fields.length} joueurs · le premier commence à gauche</p>
