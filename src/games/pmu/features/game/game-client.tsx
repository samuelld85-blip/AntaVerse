...
export function GameClient() {
  ...
  if (game.phase === "end") return <main className="game-shell safe-shell pmu-shell"><header className="game-header"><Brand compact /><QuitGameButton homeHref="/pmu" /></header><div className="pmu-status"><span>COURSE TERMINÉE</span><strong>{game.lastDraw ? `${game.lastDraw.card.value}${game.lastDraw.card.suit}` : "PMU"}</strong></div><RaceTrack game={game} /><SettlementPanel game={game} onRestart={() => router.push("/pmu/joueurs")} /></main>;
