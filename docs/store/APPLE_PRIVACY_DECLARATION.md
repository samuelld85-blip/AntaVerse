...
Dernière vérification des exigences stores : 2026-08-20. Document
...
**À revalider obligatoirement à chaque ajout de SDK natif**, en particulier
lors du passage à un wrapper natif (voir
`docs/store/NATIVE_PACKAGING_OPTIONS.md`), qui peut introduire une collecte
que la version web statique actuelle n'a pas.
...
- Le seul flux de données existant est le trafic HTTP standard entre le
  navigateur de l'utilisateur et l'hébergeur (Vercel), pour livrer les
  pages — un flux inhérent à l'hébergement web lui-même, pas une "collecte"
  au sens du formulaire Apple (qui porte sur ce que *l'application*
  collecte activement, pas sur la livraison réseau générique).
...
- L'ajout de tout SDK natif lors du passage à un wrapper (Capacitor ou
  équivalent) — les wrappers natifs embarquent parfois des capacités de
  collecte par défaut (device identifiers, crash reporting) qu'il faudra
  auditer spécifiquement, indépendamment de ce document.
