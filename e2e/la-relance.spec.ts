...
test("joue sept manches uniques et affiche le bon résultat sur mobile", async ({ page }) => {
  ...
  await expect(page.getByText("Manche 1 / 7")).toBeVisible();
  ...
  for (let round = 0; round < 7; round += 1) {
    ...
    const winner = round < 4 ? "Les Verts" : "Les Bleus";
    ...
    if (round < 6) {
      ...
      await expect(page.getByText(`Manche ${round + 2} / 7`)).toBeVisible();
    } else {
      await page.getByRole("button", { name: /voir le résultat/i }).click();
    }
  }

  expect(seenThemes.size).toBe(7);
  await expect(page.getByRole("heading", { name: /Les Verts remporte la partie/i })).toBeVisible();
  await expect(page.locator(".score-value").nth(0)).toHaveText("4");
  await expect(page.locator(".score-value").nth(1)).toHaveText("3");
});
